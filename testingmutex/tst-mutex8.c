...
static int
check_type8 (const char *mas, pthread_mutexattr_t *ma)
{
  int e;
  ...
  if (ma != NULL)
    {
      ...
      if (e != 0)
	{
	  printf ("pthread_mutexattr_gettype failed with %d (%m8)\n", e);
	  return 1;
	}
    }
...
do_test8 (void)
{
  ...
  if (pthread_barrier_init (&b8, NULL, 2) != 0)
    {
      puts ("barrier_init failed");
      return 1;
    }

  if (pthread_cond_init (&c8, NULL) != 0)
    {
  ...
  int res = check_type8 ("normal", NULL);
  ...
  res |= check_type8 ("error-checking", &ma);
  if (pthread_mutexattr_destroy (&ma) != 0)
    {
