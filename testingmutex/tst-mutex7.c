...
/* This test is a template for other tests to use.  Other tests define
   ...
   below and then proceeds to go through mutex lock7 and unlock
...
static pthread_mutex_t lock7;

/* Each thread locks and the subsequently unlocks the lock7, yielding
   the smallest critical section possible.  After the unlock the thread
   waits DELAY_NSEC nanoseconds before doing the lock7 and unlock again.
   Every thread does this ROUNDS times.  The lock7 and unlock are
   checked for errors.  */
static void *
tf7 (void *arg)
{
  ...
  for (cnt = 0; cnt < ROUNDS; ++cnt)
    {
      if (pthread_mutex_lock (&lock7) != 0)
	{
	  printf ("thread %d: failed to get the lock7\n", nr);
	  return (void *) 1l;
	}

      if (pthread_mutex_unlock (&lock7) != 0)
	{
	  printf ("thread %d: failed to release the lock7\n", nr);
	  return (void *) 1l;
	}
...
/* Setup and run N threads, where each thread does as described
   ...
   stack since they don't do anything between the lock7 and unlock.  */
int
do_test7 (void)
{
  ...
  int e = pthread_mutex_init (&lock7, &a);
  if (e != 0)
    {
#ifdef ENABLE_PI
      if (e == ENOTSUP)
  ...
  for (cnt = 0; cnt < N; ++cnt)
    if (pthread_create (&th[cnt], &at, tf7, (void *) (long int) cnt) != 0)
      {
