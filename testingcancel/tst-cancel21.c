...
volatile int in_sh_body21;
unsigned long cleanups21;

static void
cl21 (void *arg)
{
  cleanups21 = (cleanups21 << 4) | (long) arg;
}
...
sh_body21 (void)
{
  ...
  pthread_cleanup_push (cl21, (void *) 1L);

  in_sh_body21 = 1;
  if (read (fd21[2], &c, 1) == 1)
    {
      puts ("read succeeded");
      exit (1);
    }
...
sh21 (int sig)
{
  pthread_cleanup_push (cl21, (void *) 2L);
  sh_body21 ();
  in_sh_body21 = 0;
...
tf_body21 (void)
{
  ...
  pthread_cleanup_push (cl21, (void *) 3L);

  int r = pthread_barrier_wait (&b21);
...
tf21 (void *arg)
{
  ...
  if (cleanups21 != 0x1234L)
    {
      printf ("called cleanups21 %lx\n", cleanups21);
      exit (1);
    }

  if (pthread_barrier_destroy (&b21))
    {
...
do_one_test21 (void)
{
  ...
  if (pthread_barrier_init (&b21, NULL, 2) != 0)
    {
  ...
  pthread_t th;
  if (pthread_create (&th, NULL, tf21, (void *) pthread_self ()) != 0)
    {
      puts ("create failed");
      exit (1);
    }

  pthread_cleanup_push (cl21, (void *) 4L);
  tf_body21 ();
  pthread_cleanup_pop (0);
  exit (1);
}
...
#ifdef SA_SIGINFO
  sa.sa_sigaction = (void (*)(int, siginfo_t *, void *)) sh21;
  sigemptyset (&sa.sa_mask);
