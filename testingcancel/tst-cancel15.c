...
static pthread_barrier_t bar15;
static sem_t sem15;
...
tf15 (void *arg)
{
  ...
  pthread_cleanup_push (cleanup15, NULL);

  e = pthread_barrier_wait (&bar15);
  if (e != 0 && e != PTHREAD_BARRIER_SERIAL_THREAD)
    {
      puts ("error: tf15: 1st barrier_wait failed");
      exit (1);
    }
...
do_test15 (void)
{
  ...
  if (pthread_barrier_init (&bar15, NULL, 2) != 0)
    {
  ...
  if (pthread_create (&th, NULL, tf15, NULL) != 0)
    {
      puts ("error: create failed");
      exit (1);
    }

  int e = pthread_barrier_wait (&bar15);
  if (e != 0 && e != PTHREAD_BARRIER_SERIAL_THREAD)
    {
