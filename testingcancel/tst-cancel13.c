...
static pthread_barrier_t bar13;
static sem_t sem13;
...
tf13 (void *arg)
{
  pthread_cleanup_push (cleanup13, NULL);

  int e = pthread_barrier_wait (&bar13);
  if (e != 0 && e != PTHREAD_BARRIER_SERIAL_THREAD)
    {
      puts ("error: tf13: 1st barrier_wait failed");
      exit (1);
    }
...
do_test13 (void)
{
  ...
  if (pthread_barrier_init (&bar13, NULL, 2) != 0)
    {
  ...
  if (sem_init (&sem13, 0, 0) != 0)
    {
      puts ("error: sem_init failed");
      exit (1);
    }

  if (pthread_create (&th, NULL, tf13, NULL) != 0)
    {
