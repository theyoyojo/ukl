...
static pthread_barrier_t b25;
static pthread_t th25b;
...
tf25b (void *arg)
{
  ...
  int e = pthread_barrier_wait (&b25);
...
unwhand25 (void *arg)
{
  if (pthread_create (&th25b, NULL, tf25b, NULL) != 0)
    {
      puts ("unwhand25: create failed");
      exit (1);
    }
}
...
tf25a (void *arg)
{
  pthread_cleanup_push (unwhand25, NULL);
...
do_test25 (void)
{
  if (pthread_barrier_init (&b25, NULL, 2) != 0)
    {
  ...
  pthread_t th1;
  if (pthread_create (&th1, NULL, tf25a, NULL) != 0)
    {
      puts ("create failed");
      return 1;
    }
  ...
  /* Make sure tf25b enters nanosleep.  */
  ts.tv_sec = 0;
  ...
  if (pthread_barrier_destroy (&b25) != 0)
    {
