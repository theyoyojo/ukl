...
static int
check23 (pthread_condattr_t *condattr, int pshared, clockid_t cl)
{
  clockid_t cl2;
...
run_test23 (clockid_t cl)
{
  ...
  if (check23 (&condattr, PTHREAD_PROCESS_PRIVATE, CLOCK_REALTIME))
    return 1;
  ...
  if (check23 (&condattr, PTHREAD_PROCESS_SHARED, CLOCK_REALTIME))
    return 1;
  ...
  if (check23 (&condattr, PTHREAD_PROCESS_SHARED, cl))
    return 1;
  ...
  if (check23 (&condattr, PTHREAD_PROCESS_PRIVATE, cl))
    return 1;
...
do_test23 (void)
{
  ...
  int res = run_test23 (CLOCK_REALTIME);
...
#  endif
    res |= run_test23 (CLOCK_MONOTONIC);
# else
