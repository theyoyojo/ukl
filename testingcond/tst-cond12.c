...
static char fname12[] = "/tmp/tst-cond12-XXXXXX";
static int fd12;

static void prepare12 (void);
#define PREPARE(argc, argv) prepare12 ()
...
prepare12 (void)
{
  fd12 = mkstemp (fname12);
  if (fd12 == -1)
    {
      printf ("mkstemp failed: %m\n");
      exit (1);
    }
  add_temp_file (fname12);
  if (ftruncate (fd12, 1000) < 0)
    {
...
do_test12 (void)
{
  struct
  {
    ...
    pthread_cond_t c;
    int var;
  } *p = mmap (NULL, sizeof (*p), PROT_READ|PROT_WRITE, MAP_SHARED, fd12, 0);
  if (p == MAP_FAILED)
    {
