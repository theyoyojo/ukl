...
static char fname13[] = "/tmp/tst-cond12-XXXXXX";
static int fd13;

static void prepare13 (void);
#define PREPARE(argc, argv) prepare13 ()
...
prepare13 (void)
{
  fd13 = mkstemp (fname13);
  if (fd13 == -1)
    {
      printf ("mkstemp failed: %m\n");
      exit (1);
    }
  add_temp_file (fname13);
  if (ftruncate (fd13, 1000) < 0)
    {
...
do_test13 (void)
{
  struct
  {
    ...
    pthread_cond_t c;
    int var;
  } *p = mmap (NULL, sizeof (*p), PROT_READ|PROT_WRITE, MAP_SHARED, fd13, 0);
  if (p == MAP_FAILED)
    {
