...
static pthread_cond_t cond15 = PTHREAD_COND_INITIALIZER;
static pthread_mutex_t mut15 = PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP;
static pthread_mutex_t mut15b = PTHREAD_MUTEX_INITIALIZER;

static void *
tf15 (void *p)
{
  if (pthread_mutex_lock (&mut15) != 0)
    {
  ...
  if (pthread_mutex_unlock (&mut15b) != 0)
    {
  ...
  int err = pthread_cond_timedwait (&cond15, &mut15, &ts);
  ...
  if (pthread_mutex_unlock (&mut15) != 0)
    {
...
do_test15 (void)
{
  if (pthread_mutex_lock (&mut15b) != 0)
    {
  ...
  pthread_t th;
  int err = pthread_create (&th, NULL, tf15, NULL);
  if (err != 0)
    {
  ...
  if (pthread_cond_signal (&cond15) != 0)
    {
  ...
  err = pthread_create (&th, NULL, tf15, (void *) 1l);
  if (err != 0)
    {
