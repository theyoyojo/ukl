...
static pthread_cond_t cond9 = PTHREAD_COND_INITIALIZER;
static pthread_mutex_t mut9 = PTHREAD_ERRORCHECK_MUTEX_INITIALIZER_NP;


static void *
tf9 (void *arg)
{
  int err = pthread_cond_wait (&cond9, &mut9);
  if (err == 0)
    {
  ...
  err = pthread_cond_timedwait (&cond9, &mut9, &ts);
  if (err == 0)
    {
...
do_test9 (void)
{
  ...
  err = pthread_cond_wait (&cond9, &mut9);
  if (err == 0)
    {
      puts ("cond_wait did not fail");
      exit (1);
